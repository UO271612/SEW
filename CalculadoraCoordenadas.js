...
function convertirPrimeraCoordenada() {
    ...
    var resultado = convertirUtmALatLong(numHuso, X, Y, hemisferioNorte).split(",");

    document.getElementById("latitud1").value = resultado[0];
    document.getElementById("longitud1").value = resultado[1];
}

function convertirSegundaCoordenada() {
...
function convertirUtmALatLong(numHuso, X, Y, hemisferioNorte) {
    if (!hemisferioNorte)
        Y = 10000000 - Y;
...
function mostrarDistancia() {
    ...
    var distancia = Math.round(calcularDistancia(latitud1, longitud1, latitud2, longitud2) * 10) / 10;
